...
func (m *GetKeyResponse) validate(all bool) error {
	...
	// no validation rules for LatestKeyVersionId

	if m.LatestRotationTime != nil {

		if all {
			switch v := interface{}(m.GetLatestRotationTime()).(type) {
			case interface{ ValidateAll() error }:
				if err := v.ValidateAll(); err != nil {
					errors = append(errors, GetKeyResponseValidationError{
						field:  "LatestRotationTime",
						reason: "embedded message failed validation",
						cause:  err,
					})
				}
		...
		} else if v, ok := interface{}(m.GetLatestRotationTime()).(interface{ Validate() error }); ok {
			if err := v.Validate(); err != nil {
